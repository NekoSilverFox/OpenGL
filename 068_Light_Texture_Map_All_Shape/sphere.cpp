...
void Sphere::_genVectorVerticesAndIndices()
{
    ...
    const unsigned int num_circle       = 360.0f / _step_angle;  // 半球上需要绘制圈圈的个数
    ...
    for (unsigned long i = 0; i < num_circle; i++)
    {
        ...
        for (unsigned long j = 0; j <= num_point_circle; j++)
        {
            ...
            qDebug() << "生成本圈上的第：" << j << "个点, 当前水平角度：" << current_xz_angle << "点：" << QVector3D(x, y, z);
    ...
    const int num_points = (num_point_circle + 1) * (num_circle - 1);
    qDebug() << num_points;
    for (int i = (num_point_circle + 1); i <= num_points; i++)
    {
